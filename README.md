...
правило: пункты памяти — устойчивые ограничения; если запрос противоречит
пункту памяти, агент не подчиняется молча — **вежливо отказывает** выполнить
такой запрос, добавляет **лёгкий юмор**, ссылается на конкретный пункт и
**сообщает, что принято решение действовать по памяти**. Сам пункт памяти не
изменяет и не удаляет — памятью управляет только пользователь.
