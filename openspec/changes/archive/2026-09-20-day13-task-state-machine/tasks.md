...
- [x] 2.3 Гард в `ask_stream`/`/api/chat`: активная непаузанная
      незавершённая задача → SSE `error`-событие «Задача выполняется…»
      (ответ 200), сообщение в диалог не пишется (тест в
...
- [x] 5.1 `components/TaskTab.tsx` + тесты: описание, стадии с outputs
      ...
      «Задача»).
- [x] 5.2 `ChatPanel.tsx` + тесты: кнопки **Стоп/Продолжить** в шапке
      (стоп → pause, пауза → Продолжить), статус-строка стадий
      (4 чипа: текущая — подсвечена, пройденные ✓, done —
      финальная отметка), чип «задача» на stage-сообщениях, блокировка
      поля ввода во время стадии (запуск по описанию и instruction на
      паузе — во вкладке «Задача»).
