...
- [x] 1.1 `studio/backend/tests/test_memory.py`: переписать/расширить
...
- [x] 1.2 Коммит: `feat(day13b-task): новая схема состояния задачи
...
- [x] 2.1 `studio/backend/tests/test_agent.py`: переписать
...
- [x] 2.2 Реализация в `agent.py`: константы stage-промптов (новый
...
- [x] 3.1 `studio/backend/tests/test_api.py`: обновить task-раздел —
      ...
      пишется).
- [x] 3.2 Реализация в `main.py`: обновить роуты `/api/task/*` под
      новую схему и семантику (D9); 400/404 RU-detail.
- [x] 3.3 Коммит: `feat(day13b-task): API — новая семантика
...
- [x] 4.1 `studio/frontend/src/api.ts`: типы новой TaskState
...
- [x] 4.2 `studio/frontend/src/state.tsx`: reducer под новые события и
...
- [x] 4.3 Коммит: `feat(day13b-task): frontend api+state — новые
...
- [x] 5.1 Новый `components/TaskCard.tsx` + тесты (Vitest): заголовок
...
- [x] 5.2 `ChatPanel.tsx` + тесты: рендер TaskCard после
...
- [x] 5.3 Удаление `TaskTab.tsx`, 5-й вкладки в `ContextPanel.tsx`,
...
- [x] 5.4 Коммит: `feat(day13b-task): UI — карточка процесса в чате,
      тумблер режимов, удаление вкладки «Задача»`. Верификация:
      `npm test` + `npx tsc --noEmit` зелёные.

## 6. E2E

- [x] 6.1 `scripts/e2e_studio.py`: переписать блок «Задача» —
...
- [x] 7.1 README.md (корень): обновить секцию «День 13» (режимы
      чат/задача, карточка процесса, пошаговое исполнение, новая
      схема состояния, новые SSE-события, обновлённая API-таблица,
      статус тестов).
- [x] 7.2 `studio/backend/README.md`: обновить блок «Задача (день
      13)» — unified FSM, SSE-протокол `/api/task/run` (новые
      события), семантика эндпоинтов.
- [x] 7.3 Коммит: `docs(day13b-task): README — режимы/карточка/новая
      схема/события`.

## 8. Финальная верификация

- [x] 8.1 Полный прогон: `python -m pytest -q` (бэкенд), `npm test` +
...
- [x] 8.2 Rebuild `dist` (`npm run build`), перезапуск uvicorn,
