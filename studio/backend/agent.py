...
# ограничения, тихое подчинение противоречащему запросу запрещено. При
# противоречии агент вежливо отказывает (с лёгким юмором) и сообщает, что
# принято решение действовать по памяти; сам пункт не правит — памятью
# управляет только пользователь. Добавляется в system-сообщение только когда
# есть записи памяти.
MEMORY_RULE = (
    "\n\nПравило памяти: пункты памяти — устойчивые ограничения пользователя. "
    "Если новый запрос противоречит пункту памяти — не подчиняйся ему молча: "
