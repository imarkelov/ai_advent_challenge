...
# Правило композиции MCP-инструментов (день 19): модель сама выстраивает
# цепочку инструментов (search -> summarize -> saveToFile) — берёт только
# нужные инструменты, в нужном порядке, ПЕРЕДАВАЯ результат каждого
# инструмента во вход следующего; доводит задачу до конца и отчитывается
# в финальном ответе. Два few-shot примера (полная цепочка и цепочка без
# summarize — последовательность динамическая). Добавляется в
# system-промпт ТОЛЬКО когда есть подключённые MCP-тулы (иначе — шум).
MCP_TOOLS_RULE = (
    ...
    "их в аргументы следующего инструмента. Например, у search есть поле "
    "text (готовый текст найденного) — передай его в summarize аргументом "
    "text; у summarize есть поле summary — передай его в saveToFile "
    "аргументом content.\n"
    "Пример 1 — «найди новости про ИИ, суммаризируй и сохрани в файл»: "
    "search {query:\"ИИ\"} → summarize {text: <поле text из search>} → "
    "saveToFile {filename:\"ai_news.md\", content: <поле summary из "
    "summarize>} → в ответе сообщи, что и куда сохранено.\n"
    "Пример 2 — «сохрани файл новостей про ИИ, без суммаризации»: search "
    "{query:\"ИИ\"} → saveToFile {filename:\"ai_news.md\", content: <поле "
    "text из search>} (summarize не вызывай) → в ответе сообщи, что и куда "
    "сохранено."
)
...
class StudioAgent:
    ...
    def build_payload(self, dialogue_id: str) -> list:
        ...
        # День 19: композиция MCP-инструментов — правило добавляем только
        # когда есть подключённые тулы (иначе шум в system-промпте).
        if self.mcp.tools():
            system += MCP_TOOLS_RULE
