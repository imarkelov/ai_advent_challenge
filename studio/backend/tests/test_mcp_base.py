...
# Регресс day20-fix: реальный subprocess с PYTHONIOENCODING=cp1251
# (как Windows-консоль/пайпы) — payload с символами вне cp1251
# (\u2011, \u2192) из реального Habr-контента не должен убивать сервер.
_SERVER_SRC = '''\
import sys
...
def call(args):
    return {{"ok": "non\\u2011cp1251: \\u2011 \\u2192",
             "ascii": "fine"}}, False

_mcp_base.run_server("cp1251-check", TOOL, call)
'''.format(mcp_dir=MCP_DIR)


def test_stdout_cp1251_non_encodable_payload(tmp_path):
    srv = tmp_path / "cp1251_server.py"
    srv.write_text(_SERVER_SRC, encoding="utf-8")
    env = dict(os.environ, PYTHONIOENCODING="cp1251")
    p = subprocess.Popen([sys.executable, str(srv)],
                         stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                         stderr=subprocess.PIPE,
                         text=True, encoding="cp1251", env=env)
    try:
        for m in ({"jsonrpc": "2.0", "id": 1, "method": "initialize"},
                  {"jsonrpc": "2.0", "id": 2, "method": "tools/call",
                   "params": {"name": "emit", "arguments": {}}}):
            p.stdin.write(json.dumps(m) + "\n")
        p.stdin.flush()
        lines = [p.stdout.readline() for _ in range(2)]
    finally:
        p.stdin.close()
        p.wait(timeout=30)
        stderr = p.stderr.read()
    # Ключевой assert: процесс НЕ умер с UnicodeEncodeError.
    assert p.returncode == 0, (
        "Сервер-процесс упал (UnicodeEncodeError на stdout?); "
        "PYTHONIOENCODING не сработал как ожидалось. stderr:\n" + stderr)
    r2 = json.loads(lines[1])
    assert "isError" not in r2["result"]
    payload = json.loads(r2["result"]["content"][0]["text"])
    assert payload["ascii"] == "fine"  # ASCII-часть цела
    assert "non" in payload["ok"]  # префикс до не-кодируемого символа жив
    # Символы вне cp1251 заменены (not "\u2011"/"\u2192" в ответе),
    # но ответ — валидный JSON-RPC (спарсился выше).
